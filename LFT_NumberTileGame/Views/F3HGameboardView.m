...
+(instancetype)gameboardWithDimension:(NSUInteger)dimension cellWidth:(CGFloat)width cellPadding:(CGFloat)padding cornerRadius:(CGFloat)cornerRadius backGroundColor:(UIColor *)backgroudCorlor foregroundCorlor:(UIColor *)foregroundColor{
    ...
    [view setupBackgroundWithBackgroundColor:backgroudCorlor foregroundColor:foregroundColor];
...
- (void)setupBackgroundWithBackgroundColor:(UIColor *)background
    ...
    for(int i=0; i<self.dimension; i++){
        ...
        for(int j=0; j<self.dimension; j++){
            UIView *bkgndTile = [[UIView alloc]initWithFrame:CGRectMake(xCursor, yCursor, self.tileSideLength, self.tileSideLength)];
...
//Insert  a tile, with the poping animation
-(void)insertTileAtIndexPath:(NSIndexPath *)path withValue:(NSUInteger)value{
    ...
    F3HTileView * tile  = [F3HTileView tileForPositon:position sideLength:self.tileSideLength value:value cornerRadius:self.cornerRadius];
    ...
    [UIView animateWithDuration:TILE_EXPAND_TIME delay:TILE_POP_DELAY options:0 animations:^{
        tile.layer.affineTransform = CGAffineTransformMakeScale(TILE_POP_MAX_SCALE, TILE_POP_MAX_SCALE);
...
-(void)moveTileOne:(NSIndexPath *)startA tileTwo:(NSIndexPath *)startB toIndexPath:(NSIndexPath *)end  withValue:(NSUInteger) value{
    ...
    [UIView animateWithDuration:(PER_SQUARE_SLIDE_DURATION * 1) delay:0 options:UIViewAnimationOptionBeginFromCurrentState animations:^{
        ...
        [UIView animateWithDuration:TILE_MERGE_EXPAND_TIME animations:^{
            tileA.layer.affineTransform = CGAffineTransformMakeScale(TILE_POP_MAX_SCALE, TILE_POP_MAX_SCALE);
